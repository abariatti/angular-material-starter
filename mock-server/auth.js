...
function findUser(email){
  const db = JSON.parse(fs.readFileSync('./mock-server/data/db.json', 'UTF-8'))
...
function createUser(user){
  const db = JSON.parse(fs.readFileSync('./mock-server/data/db.json', 'UTF-8'))
  db.users.push(user);
  fs.writeFileSync('./mock-server/data/db.json', JSON.stringify(db, null, 4));
}
...
router.post('/api/authenticate', function (req, res) {
  ...
  console.log('correctly authenticated : ', user);
...
// On each request check header and verify bearer token
router.use(function (req, res, next) {
  ...
  try {
    ...
    verifyToken(token);
    console.log('access_token is accepted user is authenticated');
    next();
 } catch (err) {
   const status = 401;
